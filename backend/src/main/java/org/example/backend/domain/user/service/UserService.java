...
public class UserService extends DefaultOAuth2UserService implements UserDetailsService {
    ...
    public Boolean existsByUsername(UserRequestDTO userRequestDto) {
    ...
    public Long updateUser(UserRequestDTO userRequestDto) {
        ...
        // 조회
        UserEntity entity = userRepository.findByUsernameAndIsLockAndIsSocial(userRequestDto.getUsername(), false,
                        false)
                .orElseThrow(() -> new UsernameNotFoundException(userRequestDto.getUsername()));

        // 회원 정보 수정
        entity.updateUser(userRequestDto);
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest)
            throws OAuth2AuthenticationException { // 네이버로부터 받아온 유저 정보 객체를 받음
